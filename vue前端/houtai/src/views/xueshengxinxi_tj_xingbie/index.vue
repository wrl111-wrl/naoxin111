...
<script>
	...
	import {xueshengxinxi_tj_xingbie} from "@/api/xueshengxinxi/xueshengxinxiApi";
export default {
	name: 'user',
	data() {
		return {
	...
	methods: {
		initChart() {
			xueshengxinxi_tj_xingbie().then(res=>{
				let data = res.data
				let value = []
				data.forEach(item=>{
					value.push({name:item.aa == 1 ? '男' : '女',value:item.bb})
				})
				let xData = []
				let yData = []
				data.forEach(item=>{
					xData.push(item.aa == 1 ? '男' : '女')
					yData.push(item.bb)
				})
