...
//        qingjiashenqing.setLianxidianhua();
        String num = qingjiashenqing.getQingjiabianhao();
        System.out.println(theDayNum());
        qingjiashenqing.setQingjiabianhao(num+theDayNum());
        // 调用MyBatis-Plus的save方法插入数据到数据库
        this.save(qingjiashenqing);
        return qingjiashenqing;
    }

    public List<Map<String, Object>> getError() {
        return qingjiashenqingDao.getError();
    }


    public int theDayNum() {
        return qingjiashenqingDao.theDayNum();
    }
